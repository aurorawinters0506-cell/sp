...
import { downloadsQuery, previewFor } from "@/lib/catalog";
...
export const Route = createFileRoute("/telechargements")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "My Downloads — Smart Point" },
      {
...
function DownloadsPage() {
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6">
      ...
      {(downloads.data ?? []).length === 0 && !downloads.isPending ? (
        ...
        <ul className="panel divide-y divide-border overflow-hidden">
          {(downloads.data ?? []).map((row) => (
            <li key={row.id} className="flex items-center gap-4 p-4">
              <img
                src={previewFor(row.template.preview_variant)}
